...
bytes interleave, and each appended event SHALL occupy exactly one complete,
valid line.
...
- **THEN** every event is written as a complete, valid JSON line with no
  interleaved or corrupted bytes
- **AND** all events are present, each on its own line, once every call
  returns
