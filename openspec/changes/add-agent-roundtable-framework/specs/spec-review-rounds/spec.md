...
critique. Critiques of lower severity SHALL NOT prevent consensus, and SHALL
be carried into the consensus record as advisory findings. On consensus the
framework SHALL record a consensus event carrying the approving reviewers and
the final committed state identifier.

#### Scenario: No blocking critiques
- **WHEN** every reviewer completes its turn for a draft version and the
  highest severity raised is `major`
- **THEN** consensus is declared for that version, and the `major` findings
...
When a round ends with one or more `blocking` critiques, the framework SHALL
record a revision request carrying the blocking critiques, and SHALL prompt
the developer agent to revise the draft. Each revision SHALL be committed as a
new draft version and SHALL start a new critique round. The number of rounds
in a run SHALL be bounded by a configurable limit of at least one, defaulting
to three.

#### Scenario: Revision resolves the blocking critiques
- **WHEN** a revision round produces a draft for which no reviewer raises a
  `blocking` critique
- **THEN** consensus is declared for the revised version

#### Scenario: Revision request carries the blocking critiques
- **WHEN** a round ends with two `blocking` critiques from different reviewers
...
- **WHEN** the configured round limit is reached and `blocking` critiques
  remain outstanding
